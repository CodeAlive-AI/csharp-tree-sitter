...
/// <summary>
/// Provides the tree-sitter <see cref="TreeSitter.Language"/> for the Python grammar
...
/// <c>libtree-sitter-python</c> on demand.
/// </summary>
public static class PythonLanguage
{
    /// <summary>The canonical language name in the tree-sitter-language-pack manifest.</summary>
    public const string Name = "python";

    /// <summary>
    /// The Python <see cref="TreeSitter.Language"/>. The underlying <c>TSLanguage*</c>
    ...
    /// The native <c>libtree-sitter-python</c> library could not be loaded (build it
    /// with <c>scripts/fetch-grammar.sh python</c>).
    /// </exception>
    public static Language Language => Pack.Get(Name);

    /// <summary>
    /// Creates a new <see cref="Parser"/> bound to the Python grammar. The caller owns
